...
def write_experiment_config(
    ...
    with open(config_path, "w", encoding="utf-8") as f:
        ...
        f.write("FFT Feature Settings\n")
        f.write("--------------------\n")
        f.write(f"chunk_size: {args.chunk_size}\n")
        f.write(f"nfft: {args.nfft}\n")
        f.write(f"feature_type: log-magnitude FFT\n")
        f.write(f"window: Hann\n")
        f.write(f"normalization: DC removal + power normalization + per-sample standardization\n")
        f.write(f"max_chunks_per_file: {args.max_chunks_per_file}\n\n")

        f.write("Training Settings\n")
        f.write("-----------------\n")
        f.write(f"model: SmallFFT1DCNN\n")
